...
class Summarizer:
    ...
    def call(self, function: str, *args, ns: str = "SwingSummary"):
        """<ns>.<function>(*args), SwingSummary's by default, with the arguments and result passed as
        JSON. A dotted name is called as it is (SwingTrust.speakable)."""
        if self.ctx is None:
            with START_LOCK:
                self.ctx = MiniRacer()
        ...
        name = function if "." in function else f"{ns}.{function}"
